...
class ReceiptEvaluator:
    ...
    def extract_receipt_data(self, image_path: str) -> Tuple[bool, Optional[ReceiptData], Optional[str]]:
        ...
        try:
            ...
            try:
                ...
                extracted_data = b.ExtractReceiptTransactions(image)
    ...
    def evaluate_receipt(self, image_path: str, metadata_path: Optional[str] = None) -> ReceiptEvaluationResult:
        ...
        extraction_successful, extracted_data, extraction_error = self.extract_receipt_data(image_path)
        ...
        if not result.overall_passed:
            ...
            retry_extraction_successful, retry_extracted_data, retry_extraction_error = self.extract_receipt_data(image_path)
    ...
    def evaluate_all_receipts(self) -> List[ReceiptEvaluationResult]:
        """Evaluate all receipts in the training_wheels dataset."""
        receipt_files = self.get_receipt_files()
        results = []
        
        print(f"Found {len(receipt_files)} receipts to evaluate...")
        
        for i, (image_path, metadata_path) in enumerate(receipt_files, 1):
            print(f"Processing receipt {i}/{len(receipt_files)}: {Path(image_path).name}")
            
            try:
                result = self.evaluate_receipt(image_path, metadata_path)
...
def run_evaluation_cli(data_dir: str, results_dir: Optional[str] = None, run_id: Optional[str] = None, run_name: Optional[str] = None):
    """CLI interface to run evaluations and save results."""
    print("🚀 Starting Receipt Evaluation...")
...
def main():
    ...
    parser.add_argument(
        "--data-dir", 
        default="/Users/kevingregory/Desktop/development/python/ai-that-works/2025-11-25-multimodal-evals/data",
    ...
    # Run evaluation
    run_evaluation_cli(args.data_dir, args.results_dir, args.run_id, args.run_name)
